...
impl Compress {
    pub fn new(len: usize, level: usize) -> Compress {
        ...
        let max_search = usize::MAX; // 5 * level - 4;
    ...
    fn compress_block_very_slow(&mut self, in_data: &Vec<u8>, in_len: usize, out_data: &mut Vec<u8>, o: usize) -> Result<usize, Error> {
        ...
        loop {
            ...
            if in_pos + 16 < in_len {
                ...
                candidate_pos = best_candidate;
                run_len = best_run_len;
                match_offsets[in_pos] = (in_pos - candidate_pos) as u32;
                match_lens[in_pos] = run_len as u32;
                if run_len < 4 {
                    literal_len += 1;
                    in_pos += 1;
                    continue;
        ...
        while in_pos > 0 {
            // assume literal
            literal_count += 1;
            cost = costs[in_pos + 1] + 1;
            if literal_count >= 15 {
                if (literal_count - 15) % 255 == 0 {
                    cost += 1;
                }
            }
            in_pos -= 1;
...
mod tests {
    ...
    fn compress_decompress() {
        ...
        for level in 1..9 {
            let mut comp = Compress::new(1024, 9);
